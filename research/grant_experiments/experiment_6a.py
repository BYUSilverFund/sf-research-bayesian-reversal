...
m_fac_rets = (
    ...
    .agg(
        [
            (((pl.col(f) / 100 + 1).product() - 1) * 100).alias(f)
            for f in quality_factors
        ]
    )
)

# Create features and targets for timing
# Target is the next month's return for a specific factor
...
# Walk-forward loop for factor timing
for i in range(window_months, len(unique_dates)):
    ...
    window_df = timing_ready_df.filter(pl.col("date").is_in(train_dates))
    ...
    # We must run a separate BMA regression to predict each factor
    for target_factor in quality_factors:
        y_w = (
            window_df.get_column(f"{target_factor}_target")
            * window_df.get_column("sqrt_w")
        ).to_numpy()
        ...
        # Calculate the expected forecasted return for this factor next month
        expected_return = 0.0

        latest_X = window_df.row(-1, named=True)

        for idx, m in enumerate(model_stats):
            model_forecast = m["params"].get("const", 0.0)
            for f in m["factors"]:
                model_forecast += m["params"][f] * latest_X[f]

            expected_return += model_forecast * pmp[idx]
